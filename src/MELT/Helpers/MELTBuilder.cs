...
namespace MELT
{
    public static class MELTBuilder
    {
        ...
        {
            if (loggerFactory is DisposableTestLoggerFactory disposableTestLoggerFactory) return disposableTestLoggerFactory.Sink;

            throw new ArgumentException($"The {nameof(ILoggerFactory)} provided has not be created with {nameof(MELTBuilder)}.{nameof(CreateTestLoggerFactory)}().", nameof(loggerFactory));
        }
